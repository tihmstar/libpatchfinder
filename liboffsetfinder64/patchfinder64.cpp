...
#define findstr(str,hasNullTerminator) _vmem->memmem(str, sizeof(str)-(hasNullTerminator == 0))
...
patchfinder64::patchfinder64(const char* filename) :
        _freeBuf(true),
        _buf(NULL),
        _bufSize(0),
        _entrypoint(0),
...
__attribute__((always_inline)) struct symtab_command *patchfinder64::getSymtab(){
    if (!__symtab){
        try {
...
void patchfinder64::loadSegments(){
...
loc_t patchfinder64::find_sym(const char *sym){
    uint8_t *psymtab = _buf + _symtab->symoff;
    uint8_t *pstrtab = _buf + _symtab->stroff;
...
loc_t patchfinder64::find_syscall0(){
...
loc_t patchfinder64::find_literal_ref(loc_t pos, int ignoreTimes){
    vmem adrp(*_vmem,VM_PROT_EXECUTE);
...
//}
//
loc_t patchfinder64::find_rootvnode() {
    return find_sym("_rootvnode");
}

loc_t patchfinder64::find_allproc(){
